...
//TABLE MODELS
import wh.model.PsdToHtmlOrdernow;
 ...
 */
public class PsdToHtmlAction extends org.apache.struts.action.Action {
        ...
        try
        {
            ...
            if(mandatory_check!=null && mandatory_check.equals("success"))
            {
                if(uri!=null)
                {
                    ...
                    //PSD TO HTML BASE TABLE
                    PsdToHtmlOrdernow psdtohtmlModel = new PsdToHtmlOrdernow();
                    ...
                    //
                    if(uri.contains(ApiConstants.API_ADD_PSDTOHTML_ORDER))
                    {
                        ...
                        while(itr.hasNext())
                        {
                            ...
                            if(key!=null && !key.equals("null"))
                            {
                                ...
                                //ORDERNOW TABLE ENTRIES
                                else if(key.equalsIgnoreCase("numberoflandingpage"))
                                {
                                    psdtohtmlModel.setNumberOfLandingPage((Integer)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("numberofinnergpage"))
                                {
                                    psdtohtmlModel.setNumberOfInnerPage((Integer)aftrValidateParams.get(key));
                                }
                            }
                        }
                        try
                        {
                            ...
                            //ADD BOTH FREE/PAID OPTION ID TO PsdToHtmlOrdernow TABLE
                            psdtohtmlModel.setContactDetailsId(contactDetailsId);
                            psdtohtmlModel.setFreeAddonId(freeOptionId);
                            psdtohtmlModel.setPaidAddonId(paidOptionId);
                            Long orderId = cb.addData(psdtohtmlModel);
                            ...
                            logg.log(Level.INFO, "User Psd to HTML order placed successfully and send mail to user::: result:"+result);
                        }
                        catch (Exception e)
                        {
                            result.put("uri",uri);
                            result.put("code",ApiConstants.API_ADDING_ERROR_IN_PSDTOHTMLORDER);
                            result.put("result","failure"); 
                            result.put("message","Problem while place you order with workhtml");
                            logg.log(Level.SEVERE, "ERROR WHILE ADD DATA:@PsdToHtmlOrdernow table row \n\n RESULT :::"+result);
                            e.printStackTrace();
                        }
                        return result;
                    }
                    else if(uri.contains(ApiConstants.API_GET_PSDTOHTML_ORDER_DETAILS))
                    {
                        //uri : client/get
                        List details = cb.getDetails("PsdToHtmlOrdernow");
                        result.put("uri",uri);
                        result.put("code","200");
