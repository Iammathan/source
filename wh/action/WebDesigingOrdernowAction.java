 ...
 * @author meganath
 */
public class WebDesigingOrdernowAction extends org.apache.struts.action.Action {
    ...
    public HashMap process(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException
    {
        ...
        try
        {
            ...
            if(mandatory_check!=null && mandatory_check.equals("success"))
            {
                ...
                {   
                    ...
                    //WEB DESIGN MODEL
                    WebDesignOrdernow weborderModel = new WebDesignOrdernow();
                    
                    if(uri.contains(ApiConstants.API_ADD_WEBDESIGN_ORDERNOW_DATA))
                    {
                        ...
                        while(itr.hasNext())
                        {
                            ...
                            if(key!=null && !key.equals("null"))
                            {
                                ...
                                else if(key.equalsIgnoreCase("isrevamp"))
                                {
                                    weborderModel.setIsRevamp((Boolean)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("isquoted"))
                                {
                                    weborderModel.setIsQuoted((Boolean)aftrValidateParams.get(key));
                                }
                            }
                        }
                        try
                        {
                            ...
                            logg.log(Level.INFO, "*****************DATA ADDING PROCESS FOR WEBDESIGN ORDER NOW PAGE*****************");
                            ...
                            //SET CHILD TABLE ID TO MAIN TABLE(@WebDesignOrdernow)
                            weborderModel.setHtmlFreeAddonId(freeOptionId);
                            weborderModel.setHtmlPaidAddonId(paidOptionId);
                            weborderModel.setContactDetailsId(contactDetailsId);
                            
                            Long id = cb.addData(weborderModel);
                            result.put("id",id);
                            result.put("uri",uri);
                            result.put("code","200");
                            result.put("result","success"); 
                            result.put("message","Web desigining order placed sucessfully");
                            logg.log(Level.INFO, "User Webdesign order placed successfully and send mail to user::: result:"+result);
                             /*
                        ...
                        {
                            result.put("uri",uri);
                            result.put("code",ApiConstants.API_ADDING_ERROR_IN_WEBDESIGNINIG_ORDER);
                            result.put("result","failure"); 
                            result.put("message","Problem while adding webdesign order details");
                            logg.log(Level.SEVERE, "ERROR WHILE ADD DATA:@WebDesignOrdernow table row \n\n RESULT :::"+result);
                            e.printStackTrace();
                        }
                        return result;
                    }
                    else if(uri.contains(ApiConstants.API_GET_WEBDESIGINING_ORDER))
                    {
                        //uri : client/get
                        List details = cb.getDetails("WebDesigningOrdernow");
                        result.put("uri",uri);
                        result.put("code","200");
        ...
        {
            logg.log(Level.SEVERE, "ERROR WHILE PROCESSING ODER DETAILS IN WEBDESIGN ORDER");
            e.printStackTrace();
            Integer errcode = ApiConstants.API_MANDATORY_FIELDS_ARE_NOT_FOUND_CODE;
            String errmsg = ApiConstants.API_MANDATORY_FIELDS_ARE_NOT_FOUND_MSG;
