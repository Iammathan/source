 ...
 */
public class ClientContactDetailsAction extends org.apache.struts.action.Action {
    ...
    public HashMap process(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException
    {
        ...
        try
        {
            ...
            if(mandatory_check!=null && mandatory_check.equals("success"))
            {
                if(uri!=null)
                {
                    ClientContactDetails cdetails = new ClientContactDetails();
                    if(uri.contains(ApiConstants.API_ADD_CLIENT_DETAILS))
                    {
                        // uri : client/add
                        ...
                        while(itr.hasNext())
                        {
                            ...
                            if(key!=null && !key.equals("null"))
                            {
                                ...
                                {
                                    cdetails.setName((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("email"))
                                {
                                    cdetails.setEmail((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("phonenumber"))
                                {
                                    cdetails.setPhoneNumber((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("countrycode"))
                                {
                                    cdetails.setCountryCode((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("projectbreif"))
                                {
                                    cdetails.setProjectBreif((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("addedlinks"))
                                {
                                    cdetails.setAddedLinks((String)aftrValidateParams.get(key));
                                }
                                else if(key.equalsIgnoreCase("isattachment"))
                                {
                                    cdetails.setIsAttachments((Boolean)aftrValidateParams.get(key));
                                }
                            }
                        }
                        try
                        {
                            Long id = cb.addData(cdetails);
                            result.put("id",id);
                            result.put("uri",uri);
                            result.put("code","200");
                        ...
                        {
                            result.put("uri",uri);
                            result.put("code",ApiConstants.API_ADDING_ERROR_IN_CLIENT_DETAILS);
                    ...
                    else if(uri.contains(ApiConstants.API_GET_CLIENT_DETAILS))
                    {
                        //uri : client/get
                        List details = cb.getDetails("ClientContactDetails");
                        result.put("uri",uri);
                        result.put("code","200");
